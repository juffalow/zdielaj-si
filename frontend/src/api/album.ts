import { post, get, patch, httpDelete } from './client';
...
export async function getCurrentUserAlbums(first = 10, after = 0): Promise<any> {
  return get<any>(`${process.env.REACT_APP_API_URL}/me/albums?first=${first}&after=${after}`)
    .then((response) => {
