...
export async function getPublicProfile(id: ID): Promise<PublicProfile> {
  return get<API.PublicProfiles.GetPublicProfileResponse>(`${import.meta.env.VITE_API_URL}/publicprofiles/${id}`).then(
    (response) => {
...
export async function createPublicProfile(id: string, name: string, description: string): Promise<PublicProfile> {
  return protectedPost<API.PublicProfiles.CreatePublicProfileResponse>(
    `${import.meta.env.VITE_API_URL}/publicprofiles/`,
    { id, name, description }
...
export async function updatePublicProfile(
  ...
  params: {
    name?: string;
    description?: string;
    contact?: { homepage?: string; facebook?: string; instagram?: string; pinterest?: string; strava?: string };
  }
): Promise<PublicProfile> {
  return protectedPatch<API.PublicProfiles.UpdatePublicProfileResponse>(
    `${import.meta.env.VITE_API_URL}/publicprofiles/${id}`,
    params
  ).then((response) => response.data.publicProfile);
}
...
export async function getPublicProfileAlbums(params: {
  publicProfileId: ID;
  first?: number;
  after?: number;
}): Promise<any> {
  ...
  return get<API.GetPublicProfileAlbumsResponse>(
    `${import.meta.env.VITE_API_URL}/publicprofiles/${publicProfileId}/albums?${searchParams}`
  ).then((response) => {
