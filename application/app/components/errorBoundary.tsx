...
class ErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(
    error: Error | (Error & { response: { url: string; headers: { 'x-amzn-requestid': string, 'x-amzn-trace-id': string } } }),
    errorInfo: ErrorInfo
  ) {
    ...
    Sentry.captureException(error, {
      extra: {
        requestId: 'response' in error && 'headers' in error.response ? error.response.headers['x-amzn-requestid'] : null,
      ...
      return (
        <Alert color="danger">
          <h2 className="mb-2 text-2xl font-bold">
            {
              'message' in this.state.error && typeof this.state.error.message === 'string' && this.state.error.message !== null ? (
                this.state.error.message
              ) : 'Unknown error'
            }
          </h2>

          <p>Error was reported to the support team. If the problem persists, please contact us at <a href="mailto:info@zdielaj.si">info@zdielaj.si</a>.</p>

          <Divider className="my-4" />

          <div className="w-full clearfix">
            <Button color="danger" size="sm" className="float-end" onPress={this.onRefresh}>Refresh</Button>
            <Button color="danger" size="sm" className="me-2 float-end" onPress={this.onClick}>More info</Button>
          </div>
