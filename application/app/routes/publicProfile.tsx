...
export function meta() {
  return [{ title: 'Public profile' }, { name: 'description', content: 'Public profile' }];
}
...
export default function PublicProfile() {
  ...
  const loadMore = useCallback(() => {
    setAlbumPromises([
      ...albumPromises,
      getPublicProfileAlbums({ publicProfileId: params.id as string, after: albumPromises.length * 8, first: 8 }),
    ]);
  ...
  return (
    <ErrorBoundary notFound={<NotFound />}>
      <Suspense fallback={<PublicProfileLoader />}>
        {publicProfilePromise !== null ? <Info publicProfilePromise={publicProfilePromise} /> : null}
      </Suspense>
