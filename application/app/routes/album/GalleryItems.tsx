      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-6 md:grid-cols-4 sm:grid-cols-2 gap-2">
          {
            files.map(file => (
              <div key={file.id} style={{ aspectRatio: '1/1' }} className="justify-self-stretch">
                ...
                <Image key={file.id} src={file.thumbnails[1] || file.thumbnails[0] || file.location} width="100%" height="100%" classNames={{ wrapper: 'h-full w-full bg-cover max-w-none', img: 'object-cover object-center' }} />
              </div>
            ))
          }
      </div>
