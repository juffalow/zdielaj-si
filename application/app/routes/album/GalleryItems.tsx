...
const GalleryImage = ({ file, ...props }: { file: AlbumFile | UploadedFile } & HTMLAttributes<HTMLElement>) => {
  ...
  if ('preview' in file) {
    return (
      ...
      >
        ...
        {
          file.isDone === false && (
              <Spinner classNames={{ base: `absolute w-full h-full inset-0 z-999 ${styles.bgGrayOpaque}` }} color={file.isUploading ? 'success' : 'primary'} />
          )
        }
        {
          file.hasError === true && (
  ...
  if ('preview' in file) {
    return (
      ...
      >
        ...
        <video src={file.preview} className="h-full w-full bg-cover max-w-none object-cover object-center rounded-sm" autoPlay />
      </div>
    );
  }
