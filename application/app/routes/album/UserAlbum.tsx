...
import { Button, Form, Alert, Input, Textarea, Select, SelectItem, Checkbox, Image } from '@heroui/react';
    ...
    <div className="grid grid-cols-3 gap-4 mt-10">
      <div className="col-span-1">
        <Form action={formAction}>
          ...
          <Select label={t("retention")} name="retention" labelPlacement="outside" defaultSelectedKeys={user === null ? ["1"] : ["0"]} isDisabled={user === null}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-6 md:grid-cols-4 sm:grid-cols-2 gap-2">
          {
            files.map(file => (
              <div key={file.name} style={{ aspectRatio: '1/1' }} className="justify-self-stretch">
                <Image src={file.preview} width="100%" height="100%" classNames={{ wrapper: 'h-full w-full bg-cover max-w-none', img: 'object-cover object-center' }} />
              </div>
            ))
          }
        </div>
