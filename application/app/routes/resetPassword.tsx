...
const ResetPassword: FunctionComponent = () => {
  ...
  const onRequestSubmit = (username: string) => {
    return resetPassword(username).then(() => {
      setStep(1);
  ...
  const onResetSubmit = (password: string, code: string) => {
    return confirmResetPassword(username, code, password).then(() => {
      navigate(
        `/${ROUTES[i18n.language as keyof typeof ROUTES].prefix}${ROUTES[i18n.language as keyof typeof ROUTES].signIn}`
      );
    });
  };
  ...
  return (
    <div className="flex min-h-full flex-col justify-center px-6 lg:px-8">
      ...
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        {step === 0 ? <RequestResetForm onRequestSubmit={onRequestSubmit} /> : null}
        {step === 1 ? <ResetForm username={username} onResetSubmit={onResetSubmit} /> : null}
      </div>
    </div>
  );
};
