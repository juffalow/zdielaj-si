...
export function meta({ location }: Route.MetaArgs) {
  ...
  return [
    ...
    language === 'en' && {
      name: 'description',
      content:
        'Do you also have a problem that some applications reduce the quality of your photos and videos? Here you can share them without any problems in full quality!',
    },
    language === 'sk' && {
      name: 'description',
      content:
        'Máte problém, že niektoré aplikácie snížujú kvalitu vašich fotiek a videí? Tu ich môžete bezproblémov zdieľať v plnej kvalite!',
    },
    language === 'cz' && {
      name: 'description',
      content:
        'Také máš problém, že ti některé aplikace snižují kvalitu fotek a videí? Tady je můžeš sdílet bez problémů v plné kvalitě!',
    },
    language === 'de' && {
      name: 'description',
      content:
        'Haben Sie auch das Problem, dass einige Anwendungen die Qualität Ihrer Fotos und Videos reduzieren? Hier können Sie sie problemlos in voller Qualität teilen!',
    },
    language === 'es' && {
      name: 'description',
      content:
        '¿Tienes problemas con que algunas aplicaciones reduzcan la calidad de tus fotos y videos? Aquí puedes compartirlos sin problemas en calidad completa!',
    },
    language === 'fr' && {
      name: 'description',
      content:
        'Vous avez également un problème que certaines applications réduisent la qualité de vos photos et vidéos? Voici comment les partager sans problèmes en qualité complète!',
    },
    language === 'it' && {
      name: 'description',
      content:
        'Hai problemi con alcune applicazioni che riducono la qualità delle tue foto e video? Qui puoi condividerli senza problemi in qualità completa!',
    },
    language === 'pl' && {
      name: 'description',
      content:
        'Masz problem, że niektóre aplikacje obniżają jakość Twoich zdjęć i filmów? Tutaj możesz je udostępnić bez żadnych problemów w pełnej jakości!',
    },
    language === 'nl' && {
      name: 'description',
      content:
        "Hebt u ook het probleem dat sommige toepassingen de kwaliteit van uw foto's en video's verlagen? Hier kunt u ze probleemloos in volledige kwaliteit delen!",
    },
    language === 'si' && {
      name: 'description',
      content:
        'Imate težave, da nekatere aplikacije zmanjšujejo kakovost vaših foto in video? Tukaj jih lahko delite brez težav v celotni kakovosti!',
    },
    language === 'fi' && {
      name: 'description',
      content:
        'Onko sinilkojalla, että joitakin sovelluksia vähentävät kuvan ja videon laatua? Voit jakaa ne ilman ongelmia täysikokoisella laadulla!',
    },
    language === 'se' && {
      name: 'description',
      content:
        'Har du problem att vissa apparater minskar kvalitén på dina foton och videor? Här kan du dela dem utan problem i full kvalitet!',
    },
    language === 'no' && {
      name: 'description',
      content:
        'Har du problem med at noen applikasjoner reduserer kvaliteten på dine bilder og videoer? Her kan du dele dem uten problemer i full kvalitet!',
    },
    language === 'dk' && {
      name: 'description',
      content:
        'Har du problem med at nogle applikationer reducerer kvaliteten på dine billeder og videoer? Her kan du dele dem uden problemer i fuld kvalitet!',
    },
    language === 'hu' && {
      name: 'description',
      content:
        'Van probléma, hogy néhány alkalmazás csökkenti a fényképek és videók minőségét? Itt tudod megosztani őket problém nélkül teljes minőségben!',
    },
  ];
}
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[], fileRejections: FileRejection[]) => {
      ...
      setTimeout(async () => {
        const { files } =
          user === null
            ? await addFilesToAlbum(
                album.id,
                album.token,
                acceptedFiles.map((file) => ({ name: file.name, mimetype: file.type, size: file.size }))
              )
            : await addFilesToUserAlbum(
                album.id,
                acceptedFiles.map((file) => ({ name: file.name, mimetype: file.type, size: file.size }))
              );
        ...
        uploadFiles(
          acceptedFiles,
          files.map((f: any) => ({ url: f.url as string, fields: f.fields }))
        );
      }, 1);
    ...
    <div>
      ...
      <div className="mt-5 p-8">
        <div
          ...
          style={{ border: '1px #28a745 dashed', padding: '40px' }}
          data-tracking-id="home_upload_area_click"
        >
